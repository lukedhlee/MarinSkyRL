...
def fsdp2_load_full_state_dict(model: torch.nn.Module, full_sd: dict, cpu_offload=None, ep_enabled=False):
    ...
    if ep_enabled:
        ...
        def _extract_local_shard(full_cpu, dtensor_meta):
            ...
            # torch>=2.9: _StridedShard lives in placement_types (tests already
            # import from there). Older nightlies briefly exposed it on
            # _dtensor_spec — keep a fallback for those.
            try:
